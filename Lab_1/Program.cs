...
namespace OnnxSample
{
    class Program
    {
        ...
        {
            var image = Image.Load<Rgb24>(args.FirstOrDefault() ?? "image.jpg");
            //?? -оператор проверки на не ноль
            //args.FirstOrDefault() - первый аргумент строки
            ...
            var input = new DenseTensor<float>(new[] { 1, 3, TargetHeight, TargetWidth }); //тройка тк три матрицы на каждый из цветов
            var mean = new[] { 0.485f, 0.456f, 0.406f };
            var stddev = new[] { 0.229f, 0.224f, 0.225f }; //что это???
            for (int y = 0; y < TargetHeight; y++)
            {
                Span<Rgb24> pixelSpan = image.GetPixelRowSpan(y);
                for (int x = 0; x < TargetWidth; x++)
                {
                    input[0, 0, y, x] = ((pixelSpan[x].R / 255f) - mean[0]) / stddev[0]; // сначала приводим в [0,1], а потом производим z-нормализацию 
            ...
            var session = new InferenceSession(@"D:\7_sem_prac\Lab_1\Lab_1\shufflenet-v2-10.onnx");
